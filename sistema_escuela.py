...
def resource_path(relative_path):
    ...
    try:
        ...
        # Si no se está ejecutando como .exe, usa la ruta normal
...
try:
    from PyQt6.QtWidgets import (
        ...
        QMenu, QSizePolicy, QSpacerItem, QListWidget, QListWidgetItem, QDialog # Necesario para el nuevo menú
    )
    # --- AÑADIDO QLocale ---
...
# Esta es la versión de tu .exe actual. DEBES incrementarla cada vez que compiles.
...
try:
    ...
    from reportlab.lib.units import inch, mm # Asegúrate de tener 'mm' aquí también
...
try:
    ...
    from PIL import Image as PILImage
     ...
     print("Advertencia: Pillow no está instalado. Funciones de imagen limitadas.")

try:
    # Intenta importar la librería para manejar el escáner en Windows
    import win32com.client
    WIA_AVAILABLE = True
except ImportError:
    WIA_AVAILABLE = False
    print("Advertencia: pywin32 no está instalado. La función de escaneo directo no estará disponible.")

# ---- Configuración de la Base de Datos y Archivos ----
# ---- Configuración de Rutas Absolutas ----
# Obtiene la ruta donde está guardado este archivo .py
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

if getattr(sys, 'frozen', False):
    # Corriendo como .EXE: El directorio base es donde está el .exe
    BASE_DIR = os.path.dirname(sys.executable)
else:
    # Corriendo como .PY: El directorio base es donde está el script
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
...
# Si usas background.jpg, añádelo también:
...
def inicializar_db():
    ...
    try:
        ...
        # --- Alteraciones para Cursos (Si la tabla ya existía sin los nuevos campos) ---
        try:
        ...
        # Si falla algo crítico, mostrarlo
...
class VentanaDocumentos(QDialog):
    def __init__(self, alumno_id, nombre_alumno, parent=None):
        ...
        self.nombre_alumno = nombre_alumno  # <--- GUARDAR EL NOMBRE AQUÍ
        self.setWindowTitle(f"Expediente Digital - {nombre_alumno}")
        self.setMinimumSize(750, 500) # Un poco más ancha para los nuevos botones
        self.tipos_documentos = ["Acta de Nacimiento Alumno", "Acta de Nacimiento Tutor", "CURP Alumno", "CURP Tutor", "Comprobante de Domicilio", "Certificado Médico Alumno", "INE Tutor"]
    ...
    def init_ui(self):
        ...
        # Ajustamos las columnas: Tipo, Estado, Acciones (ahora más ancha)
        ...
        for i, tipo in enumerate(self.tipos_documentos):
            ...
            # --- Widget contenedor para los botones de acción ---
            ...
            btn_escanear = QPushButton("🖨️ Escanear")
            btn_escanear.setToolTip("Conectar al escáner para digitalizar ahora")
    ...
    def cargar_estado_documentos(self):
        """Revisa en la BD qué documentos ya existen."""
        try:
            ...
            for i, tipo in enumerate(self.tipos_documentos):
                if tipo in docs_registrados and os.path.exists(docs_registrados[tipo]):
                    self.tabla_docs.item(i, 1).setText("Cargado ✅")
                    self.tabla_docs.item(i, 1).setForeground(QColor("green"))
                    
                    # Reemplazar botones de carga/escaneo con botón de "Ver"
                    btn_ver = QPushButton("👁️ Ver Documento")
    ...
    def procesar_y_guardar_archivo(self, ruta_origen, tipo_doc, es_escaneo_temporal=False):
        """Función centralizada para guardar el documento en la carpeta final y la BD."""
        try:
            ...
            if es_escaneo_temporal:
                 ...
                 # --- CORRECCIÓN CLAVE: USAR PILImage EN LUGAR DE Image ---
                 image = PILImage.open(ruta_origen) 
                 
                 # Convertir a RGB si es necesario (para que la conversión a PDF sea exitosa)
                 if image.mode == 'RGBA':
                ...
                # Si es carga directa, mantenemos la extensión original
            ...
            self.cargar_estado_documentos()
            QMessageBox.information(self, "Éxito", f"Documento '{tipo_doc}' guardado correctamente.")
    ...
    def escanear_documento(self, tipo_doc):
        """Conecta con el escáner usando WIA (Windows)."""
        if not WIA_AVAILABLE:
            QMessageBox.warning(self, "Error", "Librería 'pywin32' no instalada.")
            return

        try:
            # Crear el diálogo WIA
            ...
            imagen_wia = wia_dialog.ShowAcquireImage(
                DeviceType=1,
                Intent=1, 
                FormatID="{B96B3CAE-0728-11D3-9D7B-0000F81EF32E}" # Formato JPEG (más compatible)
            )
            ...
            # El error "COM Error -2145320959" suele significar que el usuario canceló el diálogo.
            if "-2145320959" in str(e): 
                pass # Usuario canceló, no hacemos nada
            else:
                QMessageBox.critical(self, "Error de Escáner", f"No se pudo conectar al escáner.\nAsegúrese de que está encendido y conectado.\nDetalle: {e}")
    ...
    def cargar_estado_documentos(self):
        ...
        try:
            ...
            for i, tipo in enumerate(self.tipos_documentos):
                if tipo in docs_registrados and os.path.exists(docs_registrados[tipo]):
                    item_estado = QTableWidgetItem("Cargado ✅")
                    item_estado.setForeground(QColor("green"))
                    item_estado.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                    self.tabla_docs.setItem(i, 1, item_estado)
                    
                    # Cambiar botón a "Ver/Abrir"
                    btn_ver = QPushButton("👁️ Ver Doc")
                    ruta = docs_registrados[tipo]
                    btn_ver.clicked.connect(lambda checked, r=ruta: QDesktopServices.openUrl(QUrl.fromLocalFile(r)))
                    self.tabla_docs.setCellWidget(i, 2, btn_ver)
        except Exception as e:
    ...
    def cargar_documento(self, tipo_doc):
        """Abre un diálogo para seleccionar y guardar un documento."""
        ruta_origen, _ = QFileDialog.getOpenFileName(self, f"Seleccionar {tipo_doc}", "", "Documentos (*.pdf *.jpg *.png *.jpeg)")
        if ruta_origen:
            try:
                ...
                # 4. Guardar en BD (Insertar o Actualizar si ya existía)
                with get_db_connection() as conn:
                    cursor = conn.cursor()
                ...
                QMessageBox.information(self, "Éxito", f"{tipo_doc} cargado correctamente.")

            except Exception as e:
                QMessageBox.critical(self, "Error", f"No se pudo guardar el documento: {e}")

# --- (Añadir esta clase cerca de VentanaDocumentos) ---
# (Asegúrate de que esto esté cerca de tu otra clase VentanaDocumentos)
...
class VentanaDocumentosPersonal(QDialog):
    def __init__(self, personal_id, nombre_personal, parent=None):
        ...
        self.tipos_documentos = [
            "Acta de Nacimiento", "CURP", "RFC", "Asignación de NSS",
            "INE (Anverso)", "INE (Reverso)", "Comprobante de Estudios",
            "Comprobante de Domicilio", "Carta de Recomendación 1", "Carta de Recomendación 2"
        ]
        ...
        try:
            ...
            for i, tipo in enumerate(self.tipos_documentos):
                if tipo in docs_registrados and os.path.exists(docs_registrados[tipo]):
                    ...
                    # (Lógica para resetear a "Pendiente" si el archivo fue borrado, si es necesario)
                    pass 
        except Exception as e:
...
class DialogGestionPersonal(QDialog):
    """Diálogo para registrar o editar personal."""
    def __init__(self, personal_id=None, parent=None):
        ...
        btn_tomar_foto = QPushButton("📸 Tomar Foto")
        btn_tomar_foto.clicked.connect(self.tomar_foto)
        btn_cargar_foto = QPushButton("📂 Cargar Foto")
        ...
        self.combo_estado_civil.addItems(["Soltero(a)", "Casado(a)", "Viudo(a)", "Divorciado(a)", "Unión Libre"])
        self.combo_escolaridad = QComboBox()
        self.combo_escolaridad.addItems(["Primaria", "Secundaria", "Bachillerato", "Licenciatura", "Maestría", "Doctorado"])
        ...
        form_contacto.addRow("Dirección Completa:", self.entry_direccion)
        form_contacto.addRow("Correo Electrónico:", self.entry_email)
        form_contacto.addRow("Teléfono:", self.entry_telefono)
        ...
        layout_emergencia.addWidget(QLabel("Teléfono 1:"), 0, 2)
        ...
        layout_emergencia.addWidget(QLabel("Teléfono 2:"), 1, 2)
    ...
    def tomar_foto(self):
        # (Reutiliza la lógica de VentanaPrincipal.tomar_foto_webcam)
        # (Necesitarás pasar self.lbl_foto_registro a esa función o copiarla aquí)
        # (Por simplicidad, asumiremos que se copia la lógica de la VentanaPrincipal)
        try:
            cap = cv2.VideoCapture(0)
            if not cap.isOpened():
                mostrar_error("No se pudo acceder a la cámara web.")
                return
            while True:
                ret, frame = cap.read()
                if not ret: break
            ...
            cv2.destroyAllWindows()
             ...
             mostrar_error(f"Error con la cámara: {e}")
        ...
        try:
            with get_db_connection() as conn:
                ...
                if self.personal_id is None:
                    ...
                    cursor.execute("SELECT COUNT(id) FROM personal WHERE num_empleado LIKE ?", (f"P-{año}-%",))
                    count = cursor.fetchone()[0]
                    num_empleado = f"P-{año}-{count + 1:03d}"
                    ...
                    mostrar_info(f"Personal registrado con éxito.\nNúmero de Empleado: {num_empleado}")
                    ...
                    mostrar_info("Personal actualizado con éxito.")

                # --- Lógica de Foto (Común para ambos modos) ---
                ...
                conn.commit()
            self.accept() # Cerrar el diálogo si todo salió bien
        except sqlite3.IntegrityError as e:
            mostrar_error(f"Error: El CURP, RFC o NSS ya podrían existir en la base de datos. {e}")
        except Exception as e:
...
class DialogoInscribirCurso(QDialog):
    """
    Un nuevo diálogo para inscribir a un ALUMNO EXISTENTE en un NUEVO CURSO.
    """
    def __init__(self, alumno_id, nombre_alumno, parent=None):
        super().__init__(parent)
        self.alumno_id = alumno_id
        ...
        self.layout.addRow("Costo de Inscripción:", self.lbl_costo_total)
    ...
    def cargar_cursos_disponibles(self):
        """Carga solo los cursos en los que el alumno NO está inscrito."""
        try:
            with get_db_connection() as conn:
                cursor = conn.cursor()
                # 1. Obtener cursos en los que el alumno YA está
            ...
            if count == 0:
                ...
                self.combo_curso.addItem("Este alumno ya está en todos los cursos")
    ...
    def actualizar_planes_pago(self):
        """
        Copia la lógica de 'actualizar_opciones_pago' de la VentanaPrincipal,
        pero usa los controles de este diálogo.
        """
        try:
            ...
            # (Revisamos todos los planes igual que en la función original)
            if curso['costo_exhibicion'] > 0:
                costo = Decimal(curso['costo_exhibicion']).quantize(quantizer)
                texto = f"Una Sola Exhibición (${costo})"
                self.combo_plan_pago.addItem(texto, userData=("Exhibición Única", float(costo), 1))
            ...
            # (Incluimos la lógica de mensualidad que ya existe en el archivo base)
    ...
    def guardar_inscripcion(self):
        ...
        try:
            ...
            reply = QMessageBox.question(self, "Generar PDF",
                                         "Inscripción exitosa.\n¿Desea generar la ficha de inscripción y el calendario de pagos para este nuevo curso?",
                                         QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            
            if reply == QMessageBox.StandardButton.Yes:
                # Usamos la función de la VentanaPrincipal
                self.parent_window.generar_ficha_completa_pdf(inscripcion_id, imprimir_poliza=False)

            self.accept() # Cierra el diálogo

        except Exception as e:
            mostrar_error(f"Error al guardar la inscripción: {e}\n{traceback.format_exc()}")

# --- (PEGAR ESTA NUEVA CLASE COMPLETA) ---

class DialogoEditarAlumno(QDialog):
    """
    Un diálogo para editar los datos de un ALUMNO EXISTENTE.
    ...
    def __init__(self, alumno_id, parent=None):
        ...
        layout_emergencia.addRow("Teléfono 1:", self.entry_emerg_1_telefono)
        ...
        layout_emergencia.addRow("Teléfono 2:", self.entry_emerg_2_telefono)
        layout_emergencia.addRow("Parentesco 2:", self.entry_emerg_2_parentesco)
        self.layout_scroll.addWidget(group_emergencia)

        # --- 5. Configuración del Scroll y Botones ---
        self.layout_scroll.addStretch()
        self.scroll_area.setWidget(self.scroll_content)
        self.layout_principal.addWidget(self.scroll_area)
    ...
    def guardar_cambios_alumno(self):
        ...
        try:
            ...
            with get_db_connection() as conn:
                ...
                # 2. Actualizar la foto (solo si se cambió)
                # La ruta guardada en self.ruta_foto_temporal puede ser la original o una nueva
                if self.ruta_foto_temporal and os.path.exists(self.ruta_foto_temporal):
                    # Verificamos si la ruta es la misma que ya está en la BD
            ...
            mostrar_error("Error: El correo electrónico ya existe para otro alumno.")
        except Exception as e:
            mostrar_error(f"Error al guardar cambios: {e}\n{traceback.format_exc()}")
    
    # --- Métodos de Foto (Copiados de VentanaPrincipal) ---
...
class LoginScreen(QWidget):
    ...
    def handle_login(self):
        ...
        try:
            with get_db_connection() as conn:
                cursor = conn.cursor()
                # --- INICIO DE MODIFICACIÓN: Seleccionar 'permisos' en lugar de 'rol' ---
            ...
            if usuario:
                ...
                try:
                    ...
                    # Fallback por si el JSON está corrupto
...
class VentanaPrincipal(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.pagina_gestion_personal = self.crear_pagina_gestion_personal() # NUEVA PÁGINA

        # Añadir páginas al StackedWidget 
        ...
        self.stacked_widget.addWidget(self.pagina_gestion_personal) # ÍNDICE 9
        ...
        # Conectar botones de navegación a las páginas 
    ...
    def crear_panel_navegacion(self):
        ...
        # Botón Menú que abrirá el QMenu
        ...
        # Spacer para empujar el menú hacia arriba y el de Configuración hacia abajo
        ...
        # Asegurarse de que solo se muestren Inicio y Menú
    ...
    def show_navigation_menu(self):
        """Muestra el menú contextual con las opciones de navegación."""
        menu = QMenu(self)
        
        # Acciones principales
        menu.addAction("📝 Nueva Inscripción", lambda: self.cambiar_pagina(1))
        menu.addAction("💵 Registro de Pagos", lambda: self.cambiar_pagina(2))
        menu.addAction("👥 Lista de Alumnos", lambda: self.cambiar_pagina(3))
        
        # Sección de Administración (solo si es Admin)
        if self.permisos.get('is_admin'): # O usar permisos específicos
            menu.addSeparator()
            menu.addAction("📚 Gestionar Cursos", lambda: self.cambiar_pagina(4))
            menu.addAction("👤 Gestionar Usuarios", lambda: self.cambiar_pagina(5))
            menu.addAction("👨‍💼 Gestionar Personal", lambda: self.cambiar_pagina(9))
            menu.addAction("📖 Gestionar Materias", lambda: self.cambiar_pagina(6))
            menu.addAction("📊 Asignar Calificaciones", lambda: self.cambiar_pagina(7))
            menu.addSeparator()
            menu.addAction("⚙️ Configuración", lambda: self.cambiar_pagina(8))
            
        # Mostrar el menú justo debajo del botón "Menú"
        # Usamos mapToGlobal para obtener la posición global del botón
        # Nota: Necesitas importar QPoint
        try:
            point = self.btn_menu.mapToGlobal(QPoint(0, self.btn_menu.height()))
            menu.exec(point)
        except AttributeError:
             # Fallback si QPoint no está definido o hay error de widget
    ...
    def crear_pagina_inicio(self):
        ...
        if self.permisos.get('can_manage_courses') or self.permisos.get('is_admin'):
            ...
            btn_sc_personal = self.crear_boton_acceso("👨‍💼", "Gestionar Personal", lambda: self.cambiar_pagina(9))
    ...
    def cambiar_pagina(self, index):
        """Cambia la página visible en el QStackedWidget con una animación de deslizamiento."""
        
        # 1. Evitar recargar la misma página si ya está activa
        ...
        # 3. Lógica de preparación de datos (IGUAL QUE ANTES, PERO SIN setCurrentIndex)
        if index == 0: 
            ...
            if self.permisos.get('can_manage_subjects') or self.permisos.get('is_admin'):
                # boton_activo = ... (Si tuvieras botón para esto)
        ...
        elif index == 9: # Página de Gestión de Personal
             if self.permisos.get('is_admin'): # O el permiso que decidas
                # boton_activo = ... (No hay botón principal, se maneja por menú)
                self.cargar_tabla_personal()

        # 4. --- INICIO DE ANIMACIÓN ESTILO MACOS ---
        ...
        # Colocamos la página entrante fuera de la pantalla (a la derecha)
        widget_entrante.setGeometry(ancho, 0, ancho, alto)
        
        # La hacemos visible inmediatamente (aunque esté fuera del área visible)
        self.stacked_widget.setCurrentIndex(index)
        
        # Creamos la animación de deslizamiento
        self.animacion = QPropertyAnimation(widget_entrante, b"pos")
        self.animacion.setDuration(300)  # Duración en milisegundos (ajustable)
        self.animacion.setStartValue(QPoint(ancho, 0))
        self.animacion.setEndValue(QPoint(0, 0))
        self.animacion.setEasingCurve(QEasingCurve.Type.InOutCubic) # Curva de aceleración suave
        self.animacion.start()
        # 5. --- FIN DE ANIMACIÓN ---

        # 6. Poner estilo seleccionado al botón activo (si aplica)
        if boton_activo:
    ...
    def crear_pagina_nueva_inscripcion(self):
        ...
        layout_superior = QHBoxLayout()
        panel_foto = QVBoxLayout() # <--- ¡IMPORTANTE! Se debe crear AQUÍ, al principio.

        # --- 2. Configurar el área de la foto ---
        ...
        self.check_autoriza_salida = QCheckBox("Autoriza que el alumno se retire solo del plantel")
        self.check_imprimir_poliza = QCheckBox("Imprimir Póliza de Garantía y Términos y Condiciones")
        ...
        # --- 4. Añadir los paneles al layout superior ---
        # AHORA SÍ funcionará porque 'panel_foto' ya fue creado en el paso 1
        layout_superior.addLayout(panel_foto)
        layout_superior.addWidget(group_alumno)

        # --- 5. Añadir el layout superior al principal ---
        layout_nuevo_alumno.addLayout(layout_superior)

        # ... (El resto de tu función sigue aquí: group_tutor, group_domicilio, etc.) ...
    ...
    def tomar_foto_webcam(self):
        """Abre la cámara web para capturar una foto."""
        try:
            ...
            while True:
                ret, frame = cap.read()
                if not ret:
                    mostrar_error("Error al leer de la cámara.")
                    break
            ...
            cv2.destroyAllWindows()
             ...
             mostrar_error(f"Error con la cámara: {e}\nAsegúrese de tener opencv-python instalado.")

    def cargar_foto_desde_archivo(self):
        """Permite seleccionar una imagen existente."""
        file_path, _ = QFileDialog.getOpenFileName(self, "Seleccionar Foto", "", "Imágenes (*.png *.jpg *.jpeg *.bmp)")
        if file_path:
            self.ruta_foto_temporal = file_path
            self.mostrar_foto_en_label(self.lbl_foto_registro, file_path)

    def mostrar_foto_en_label(self, label_widget, ruta_imagen):
        """Función auxiliar para escalar y mostrar imágenes en QLabels."""
        if ruta_imagen and os.path.exists(ruta_imagen):
            pixmap = QPixmap(ruta_imagen)
            if not pixmap.isNull():
    ...
    def crear_pagina_pagos(self):
        ...
        # El combo box ahora es la lista filtrada por la entrada de búsqueda
        self.combo_inscripcion_pago = QComboBox()
        self.combo_inscripcion_pago.setPlaceholderText("--- Seleccione una inscripción ---")
        ...
        # Se usa el QComboBox estándar como única forma de selección
    ...
    def crear_pagina_lista_alumnos(self):
        ...
        self.btn_inscribir_a_curso = QPushButton("➕ Inscribir a Nuevo Curso")
        ...
        self.btn_editar_alumno = QPushButton("✏️ Editar Alumno Seleccionado")
        self.btn_editar_alumno.setObjectName("secondaryButton") # Botón de color gris/secundario
        ...
        self.btn_generar_credencial = QPushButton("🆔 Generar Credencial de Alumno")
        ...
        self.btn_abrir_expediente = QPushButton("📂 Gestionar Expediente Digital")
    ...
    def crear_pagina_gestion_usuarios(self):
        ...
        # --- INICIO DE MODIFICACIÓN: Añadir CheckBoxes de Permisos ---
        ...
        self.permisos_map = [
            ...
            ("Ver Configuración", "can_access_settings"),
    ...
    def crear_pagina_gestion_cursos(self):
        ...
        # Bisemanal (El nuevo plazo bisemanal que se corresponde con el "sem_2" o "mensual_1" erróneos en tu imagen)
        ...
        self.tabla_cursos.setHorizontalHeaderLabels(["ID", "Curso", "Exhibición", "Sem 15", "Sem 12", "Mensual 3"]) 
    ...
    # ---- PÁGINA 5: GESTIONAR USUARIOS (NUEVA CON PERMISOS) ----
    def crear_pagina_gestion_usuarios(self):
        """Crea la interfaz para la página de gestión de usuarios."""
        widget = QWidget()
        widget.setObjectName("contentPage")
    ...
    # ---- (PÁGINA 6: GESTIONAR MATERIAS - Esta debe estar aquí) ----
    def crear_pagina_gestion_materias(self):
        """Crea la interfaz para añadir o quitar materias a un curso."""
        # ... (Asegúrate de que esta función esté presente) ...
        # (El código de esta función no se muestra para abreviar,
        #  pero debe estar en tu archivo entre crear_pagina_gestion_usuarios
        #  y crear_pagina_gestion_calificaciones)
        pass # Reemplaza 'pass' con el código de la función si la borraste
    
    # ---- (PÁGINA 7: ASIGNAR CALIFICACIONES - Esta debe estar aquí) ----
    def crear_pagina_gestion_calificaciones(self):
        """Crea la interfaz para asignar calificaciones a los alumnos."""
        # ... (El código de esta función debe estar aquí) ...
        pass # Reemplaza 'pass' con el código de la función si la borraste
        
    # ---- (PÁGINA 8: CONFIGURACIÓN - Esta debe estar aquí) ----
    def crear_pagina_configuracion(self):
        """Crea la interfaz para la página de configuración."""
        # ... (El código de esta función debe estar aquí) ...
        pass # Reemplaza 'pass' con el código de la función si la borraste
    
    # ---- PÁGINA 9: GESTIÓN DE PERSONAL ----
    def crear_pagina_gestion_personal(self):
        """Crea la interfaz para la página de gestión de personal."""
        widget = QWidget()
        widget.setObjectName("contentPage")
        layout_principal = QHBoxLayout(widget)
        layout_principal.setContentsMargins(15, 15, 15, 15)
        ...
        # (Opcional: Añadir filtro por puesto si se desea)
        ...
        self.btn_abrir_expediente_personal = QPushButton("📂 Gestionar Expediente Digital (Personal)")
        ...
        self.btn_generar_credencial_personal = QPushButton("🆔 Generar Credencial de Personal")
    ...
    # --- LÓGICA DE GESTIÓN DE USUARIOS (ACTUALIZADA) ---
    ...
    def guardar_usuario(self):
        ...
        # --- INICIO DE MODIFICACIÓN: Construir diccionario de permisos ---
        permisos_dict = {}
            ...
            with get_db_connection() as conn:
                ...
                if self.usuario_seleccionado_id is None:
                    ...
                    mostrar_info(f"Usuario creado exitosamente:\n\n"
                                 ...
                                 "Por favor, guarde esta contraseña. No se puede recuperar.")
    ...
    def cambiar_password_usuario(self):
        ...
        # --- AÑADIDO: Proteger al Admin ID 1 ---
        if self.usuario_seleccionado_id == 1:
            mostrar_error("No se puede cambiar la contraseña del Administrador principal desde aquí.")
            return
    ...
    def seleccionar_usuario_para_edicion(self):
        ...
        try:
            ...
            if datos:
                ...
                # --- INICIO DE MODIFICACIÓN: Cargar permisos ---
                try:
    ...
    def limpiar_campos_usuario(self):
        ...
        try:
            if hasattr(self, 'entry_usr_nombre'):
                ...
                # --- INICIO DE MODIFICACIÓN: Limpiar y HABILITAR checkboxes ---
                if hasattr(self, 'permisos_checkboxes'):
                    for checkbox in self.permisos_checkboxes.values():
                        checkbox.setChecked(False)
                        checkbox.setEnabled(True) # Asegurarse de que estén habilitados
                # --- FIN DE MODIFICACIÓN ---
    ...
    def crear_pagina_gestion_materias(self):
        ...
        # Panel Izquierdo: Selección de Curso
        ...
        # Panel Derecho: Añadir/Eliminar Materias
        panel_der = QGroupBox("Añadir/Eliminar Materia")
        layout_der = QFormLayout(panel_der)
        layout_der.setSpacing(10)

        self.entry_nombre_materia = QLineEdit()
        self.btn_anadir_materia = QPushButton("Añadir Materia al Curso")
    ...
    def cargar_combo_cursos_materias(self):
        """Carga los cursos en el combo de la pág. de gestión de materias."""
        try:
    ...
    def anadir_materia(self):
        ...
        try:
            ...
            mostrar_error(f"Error al añadir materia: {e}")
    ...
    def eliminar_materia(self):
        ...
        reply = QMessageBox.question(self, "Confirmar Eliminación",
                                     "¿Seguro que desea eliminar esta materia?\nEsto eliminará también todas las calificaciones asociadas a ella.",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                with get_db_connection() as conn:
                    ...
                    # SQLite borrará automáticamente las calificaciones asociadas.
    ...
    # ---- PÁGINA 7: ASIGNAR CALIFICACIONES ----
    def crear_pagina_gestion_calificaciones(self):
        """Crea la interfaz para asignar calificaciones a los alumnos."""
        widget = QWidget()
        widget.setObjectName("contentPage")
        ...
        # Panel Izquierdo: Selección de Curso y Alumno
        panel_izq = QGroupBox("Selección")
        layout_izq = QVBoxLayout(panel_izq)
        ...
        # --- INICIO DE MODIFICACIÓN: Añadir selector de parcial ---
        ...
        self.tabla_calificaciones = QTableWidget(0, 4) # ID_Materia (oculto), Materia, Calificación, Observaciones
        self.tabla_calificaciones.setHorizontalHeaderLabels(["ID_Materia", "Materia", "Calificación (0-10)", "Observaciones"])
        ...
        self.tabla_calificaciones.setEditTriggers(QTableWidget.EditTrigger.DoubleClicked) # Permitir edición
        ...
        # --- INICIO DE MODIFICACIÓN: Conectar el nuevo ComboBox ---
        # (Añadimos un lambda para que llame a la función de recarga)
    ...
    def cargar_combo_cursos_calif(self):
        """Carga los cursos en el combo de la pág. de calificaciones."""
        try:
    ...
    def cargar_alumnos_para_calificar(self):
        ...
        try:
            ...
            for alumno in alumnos:
                ...
                item.setData(Qt.ItemDataRole.UserRole, alumno['id']) # Guardamos el ID de INSCRIPCIÓN
    ...
    def cargar_tabla_calificaciones(self, current_item, previous_item):
        ...
        try:
            ...
            # --- INICIO DE MODIFICACIÓN: Determinar qué parcial cargar ---
    ...
    def guardar_calificaciones(self):
        ...
        try:
            ...
            # --- INICIO DE MODIFICACIÓN: Determinar dónde guardar ---
            idx_parcial = self.combo_parcial_select.currentIndex()
            col_calif_db = f"parcial_{idx_parcial + 1}"
            col_obs_db = f"obs_{idx_parcial + 1}"
            # --- FIN DE MODIFICACIÓN ---
    ...
    def crear_pagina_configuracion(self):
        """Crea la interfaz para la página de configuración con desplazamiento."""
        widget = QWidget()
        widget.setObjectName("contentPage")
        layout_principal = QVBoxLayout(widget)
        layout_principal.setContentsMargins(0, 0, 0, 0) # Márgenes cero para el scroll

        # 1. Crear el área de desplazamiento
        scroll_area = QScrollArea()
        scroll_area.setWidgetResizable(True)
        ...
        # --- Grupo Base de Datos ---
        group_db = QGroupBox("Respaldo y Restauración de Base de Datos")
        group_db.setMaximumWidth(450)
        layout_db = QVBoxLayout(group_db)

        lbl_info_db = QLabel("Genera copias de seguridad de tu información o restaura un respaldo anterior.\nPuedes guardar el respaldo en tu carpeta de OneDrive/Google Drive para tenerlo en la nube.")
        ...
        btn_backup = QPushButton("💾 Generar Respaldo (Backup)")
        ...
        btn_restore = QPushButton("♻️ Restaurar desde Respaldo")
    ...
    def cargar_cursos_para_filtro(self):
        """Carga los cursos en el combo de filtro de la página de alumnos."""
        ...
        try:
            ...
            pass # Ignorar si el widget aún no está listo
    ...
    def mostrar_expediente_alumno(self):
        ...
        try:
            ...
            if datos_row:
                ...
                # --- CONSTRUCCIÓN DEL EXPEDIENTE (USANDO TABLA PARA DISEÑO) ---
                expediente_str = f"""
                <table width="100%" border="0" cellspacing="0" cellpadding="5">
                    <tr>
                        <td valign="top">
                            ...
                            <b>Autoriza Salida Solo:</b> {'Sí' if datos.get('autoriza_salida_solo') == 1 else 'No'}
                        </td>
    ...
    def cargar_cache_inscripciones(self):
        """Carga TODAS las inscripciones (Pagadas, Parciales, Pendientes) en una caché local."""
        self._inscripciones_cache = []
        try:
            with get_db_connection() as conn:
                cursor = conn.cursor()
            ...
            for inscripcion in inscripciones:
                # Calcular el saldo pendiente y crear el texto de visualización
                ...
                self._inscripciones_cache.append({
                    ...
                    'search_keys': texto_completo.lower() # Texto para búsqueda rápida
                })
        except sqlite3.Error as e:
            mostrar_error(f"Error al cargar la caché de inscripciones: {e}")
            
        self.actualizar_combo_desde_cache()
    
    # ELIMINADO: La función de búsqueda se elimina si el campo de búsqueda ya no está
    # def buscar_inscripcion(self, search_term):
    #     """Filtra el QComboBox de inscripciones usando la caché local."""
    ...
    def actualizar_combo_desde_cache(self, items_to_display=None):
        """
        Rellena el QComboBox con los resultados filtrados o toda la caché.
        """
        ...
        self.actualizar_detalles_pago() # Para limpiar los detalles si no hay selección


    def cargar_tabla_inscripciones_pagos(self):
        """Recarga la caché de inscripciones y actualiza el combo."""
        # Esta función ahora solo llama a cargar_cache_inscripciones
        self.cargar_cache_inscripciones()
    ...
    def actualizar_detalles_pago(self):
        ...
        try: 
            if hasattr(self, 'combo_inscripcion_pago'):
                ...
                # Deshabilitar el monto a pagar si ya está saldado
                if pendiente <= 0.001:
    ...
    def registrar_pago(self):
        ...
        try:
            with get_db_connection() as conn:
                ...
                # --- NUEVO: Obtener el tipo de plan y curso_id de la inscripción ---
                ...
                # --- INICIO DE LÓGICA DE ACTUALIZACIÓN MODIFICADA ---
                ...
                if plan_tipo == "Mensualidad":
                    ...
                    if nuevo_monto_pagado >= (monto_total_actual - 0.001):
                        # 1. Sí cubre, así que generamos la deuda del siguiente mes.
                        ...
                        # 2. El nuevo total adeudado es el total anterior + un mes más
                        nuevo_monto_total = monto_total_actual + costo_base_mensual
                    
                    # 3. Determinamos el nuevo estado (nunca será 'Pagado' a menos que pague por adelantado)
                    if nuevo_monto_pagado >= (nuevo_monto_total - 0.001):
                        nuevo_estado = 'Pagado' # Pagó este mes Y el siguiente
                    ...
                    # --- LÓGICA ORIGINAL (Para cursos fijos) ---
            ...
            opciones_recibo = ["No generar recibo", "Ticket (Impresora Térmica)", "Recibo PDF (Completo)"]
            opcion_elegida, ok = QInputDialog.getItem(self, "Generar Recibo",
                                                      "¿Qué tipo de recibo desea generar?",
            ...
            if ok and opcion_elegida != "No generar recibo":
                ...
                elif opcion_elegida == "Ticket (Impresora Térmica)":
    ...
    def actualizar_opciones_pago(self):
        ...
        try:
            ...
            if curso['costo_exhibicion'] > 0:
                ...
                texto = crear_texto_plan("Una Sola Exhibición", costo_orig, costo_final, 1, costo_final)
                self.combo_tipo_pago_registro.addItem(texto, userData=("Exhibición Única", float(costo_final), 1))
            ...
            with get_db_connection() as conn:
                ...
                if self.ruta_foto_temporal and os.path.exists(self.ruta_foto_temporal):
                    try:
                        ...
                        # 3. Definir ruta final absoluta con extensión .png
                        nombre_foto = f"foto_{alumno_id}.png"
                        ruta_final_foto = os.path.abspath(os.path.join(FOTOS_DIR, nombre_foto))
                        
                        # 4. Guardar como PNG (más compatible con reportlab)
                        img.save(ruta_final_foto, "PNG")
                ...
                # --- Bloque 1: Generación de PDF (Condicional) ---
            if reply == QMessageBox.StandardButton.Yes:
    ...
    def generar_ficha_completa_pdf(self, inscripcion_id, imprimir_poliza=False): 
        ...
        try:
            ...
            if datos['descuento_aplicado_pct'] > 0:
                # Añadimos 'inch' como cuarto argumento
            ...
            elif plan_nombre == "Bisemanal (2)": # (Añadido para completar)
                num_pagos = 2
                monto_pago = (monto_total / 2).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            elif plan_nombre == "Mensual (4)": # (Añadido para completar)
                num_pagos = 4
                monto_pago = (monto_total / 4).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            elif plan_nombre == "Mensualidad":
                # ¡AQUÍ ESTÁ TU LÓGICA NUEVA!
                num_pagos = 12 # 12 meses del año escolar
                # monto_total ya es el costo mensual, así que monto_pago es correcto
                monto_pago = monto_total
            ...
            if imprimir_poliza:
                ...
                story_terminos.append(Spacer(1, 0.2*inch))
                story_terminos.append(Paragraph("La Unidad Profesional de Asesoría y Regularización (UPASER) tiene como garantía que el alumno se quede en sus primeras 5 opciones que seleccione en el concurso ECOEMS/103 Municipios/EXAMEN DE INGRESO A UNIVERSIDAD (UNAM, IPN, UAM, UAEMex, ETC), asegurando que nuestro curso es competente bajo nuestro método de enseñanza.", styleN_terminos))
                story_terminos.append(Spacer(1, 0.2*inch))
    ...
    def generar_ticket_pago_pdf(self, pago_info):
        ...
        try:
            ...
            # --- Título ---
            titulo_texto = "<b>Unidad Profesional de Asesorías y Regularización (UPASER)<br/>Recibo de Pago</b>"
            title_p = Paragraph(titulo_texto, styleTitle)
            title_w, title_h = title_p.wrapOn(c, width - 2*inch, height)
            title_p.drawOn(c, inch, height - 0.75 * inch - title_h)
            ...
            current_y = self.draw_text_pdf(c, current_y, f"<b>Inscripción ID:</b> {pago_info['inscripcion_id']}", inch, styleN)
            ...
            current_y = self.draw_text_pdf(c, current_y, f"<b>Método de Pago:</b> {pago_info['metodo_pago']}", inch, styleN)
            
            current_y -= 0.25 * inch # Espacio
            
            current_y = self.draw_text_pdf(c, current_y, "Estado de Cuenta de la Inscripción", inch, styleH_centered)
    ...
    def generar_ticket_termico_pdf(self, pago_info):
        """Genera un PDF del tamaño de un ticket térmico (80mm)."""
        
        # Ancho estándar de impresora térmica
        TICKET_WIDTH = 80 * mm
        # Altura inicial (crecerá si es necesario)
        ...
        try:
            ...
            # Usamos un tamaño de página personalizado
            ...
            if os.path.exists(LOGO_PATH):
                try:
                    ...
                    print(f"Error al añadir logo al ticket: {e}")
            
            # 2. Encabezado
            story.append(Paragraph("Unidad Profesional de Asesorías y Regularización (UPASER)", styleHeader))
            ...
            story.append(Paragraph(f"ID Inscripción: {pago_info['inscripcion_id']}", styleNormal))
            ...
            story.append(Paragraph(f"Método de Pago: {pago_info['metodo_pago']}", styleNormalCenter))
            story.append(Paragraph("-" * 46, styleNormalCenter))
            ...
            # --- Construir el PDF ---
            # Definir el Frame (el área donde fluye el texto)
            ...
            mostrar_error(f"Error al generar el ticket térmico PDF: {e}\n{traceback.format_exc()}")
    ...
    def generar_boleta_pdf(self):
        ...
        if parcial_elegido == "Primer Parcial":
            ...
            col_obs_db = "obs_2"
        # Si es "Promedio Final", dejamos las variables vacías por ahora
        # --- FIN DE MODIFICACIÓN ---
        ...
        try:
            ...
            if not inscripciones:
                mostrar_error("Este alumno no está inscrito en ningún curso.")
                return
            ...
            if len(inscripciones) == 1:
                ...
                curso_elegido, ok = QInputDialog.getItem(self, "Seleccionar Curso",
                                                         "Este alumno está en varios cursos. ¿De cuál desea generar la boleta?",
            ...
            if not inscripcion_id_seleccionada:
                mostrar_error("No se pudo seleccionar una inscripción válida.")
                return
            ...
            with get_db_connection() as conn:
                ...
                # --- INICIO DE MODIFICACIÓN: Lógica para Promedio Final ---
                if parcial_elegido == "Promedio Final":
                    ...
                    # Lógica original para parcial individual
                    query = f"""
            ...
            # Título (MODIFICADO)
            titulo_texto = f"<b>Unidad Profesional de Asesorías y Regularización (UPASER)<br/>Boleta de Calificaciones: {parcial_elegido.upper()}</b>"
            title_p = Paragraph(titulo_texto, styleTitle)
            title_w, title_h = title_p.wrapOn(c, width - 2*inch, height)
            title_p.drawOn(c, inch, height - 0.75 * inch - title_h)
            current_y = height - 0.75 * inch - title_h - 16
            ...
            # --- INICIO DE MODIFICACIÓN: Columnas dinámicas para Promedio ---
            if parcial_elegido == "Promedio Final":
                data_tabla = [["Materia", "Parcial 1", "Parcial 2", "Promedio"]]
                colWidths = [3*inch, 1.5*inch, 1.5*inch, 1.5*inch]
            else:
                data_tabla = [["Materia", "Calificación", "Observaciones"]]
            ...
            if calificaciones:
                for calif in calificaciones:
                    # --- INICIO DE MODIFICACIÓN: Lógica de llenado de tabla ---
                    if parcial_elegido == "Promedio Final":
                        p1 = calif['parcial_1']
                        p2 = calif['parcial_2']
            ...
            # --- INICIO DE MODIFICACIÓN: Estilo dinámico ---
            if parcial_elegido == "Promedio Final":
            ...
            # --- INICIO DE MODIFICACIÓN: Título del promedio ---
            ...
            leyenda_texto = "RECUERDA: Para que puedas hacer valida tu PÓLIZA DE GARANTÍA, es necesario contar con un PROMEDIO GENERAL mínimo de 8.0."
    ...
    def generar_credencial_pdf(self):
        """Genera dos caras de la credencial (Anverso y Reverso) en una sola página de tamaño carta (letter)."""
        
        # --- DEFINICIONES CRÍTICAS DE TAMAÑO ---
        PAGE_WIDTH, PAGE_HEIGHT = letter 
        GAFETE_W, GAFETE_H = 85.6 * mm, 53.98 * mm # Medida estándar CR80 (la que ya usabas)
        ...
        try:
            ...
            # Usamos el tamaño de página 'letter' para la impresión
            c = canvas.Canvas(filename, pagesize=letter)
            
            # --- FUNCIÓN AUXILIAR DE DIBUJO DE MARCA DE AGUA Y PLACEHOLDER ---
            ...
            # --- FUNCIÓN CENTRAL PARA DIBUJAR UNA CARA ---
            def dibujar_cara_credencial(canvas_obj, x_offset, y_offset, es_reverso=False):
                
                # Definir el área de la credencial (usamos la medida fija)
                ...
                canvas_obj.drawString(14*mm, height - 10*mm, "Unidad Profesional de Asesoría y Regularización")
                ...
                if not es_reverso: # --- ANVERSO ---
                    ...
                    canvas_obj.setFont("Helvetica-Bold", 7)
                    canvas_obj.drawString(30*mm, 24*mm, "MATRÍCULA:")
                    ...
                    canvas_obj.line(width/4, firma_y, width*3/4, firma_y) # Línea centrada
            ...
            # Colocamos el reverso a la derecha del anverso, con 1cm de separación
    ...
    def abrir_dialogo_nueva_inscripcion(self):
        """
        Abre el diálogo para inscribir a un alumno existente en un nuevo curso.
        """
        selected_items = self.tabla_alumnos.selectedItems()
        if not selected_items or self.tabla_alumnos.currentRow() < 0:
            mostrar_error("Por favor, seleccione un alumno de la lista.")
        ...
        try:
            ...
            if dialogo.exec(): # Si el diálogo se acepta (si se guardó)
                mostrar_info("Alumno inscrito a nuevo curso exitosamente.")
                # Recargamos la lista de alumnos (para actualizar el filtro)
                # y el combo de pagos (para que aparezca la nueva inscripción)
                self.cargar_tabla_alumnos()
                self.cargar_cache_inscripciones() # Recarga el combo de la pág de pagos
                
        except Exception as e:
            mostrar_error(f"Error al abrir diálogo de inscripción: {e}")
    
    def abrir_dialogo_editar_alumno(self):
        """
        Abre el diálogo para editar un alumno existente.
        """
        selected_items = self.tabla_alumnos.selectedItems()
        if not selected_items or self.tabla_alumnos.currentRow() < 0:
        ...
        try:
            ...
            mostrar_error(f"Error al abrir diálogo de edición: {e}")
    ...
    def dar_baja_curso(self):
        ...
        try:
            ...
            reply = QMessageBox.question(self, "Confirmar Eliminación",
                                         f"¿Está seguro que desea eliminar al curso:\n\n{nombre_curso} (ID: {curso_id})?\n\nEsta acción no se puede deshacer.",
                                         QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.Cancel)
    ...
    def limpiar_campos_usuario(self):
        ...
        try:
            if hasattr(self, 'entry_usr_nombre'):
                ...
                # --- INICIO DE MODIFICACIÓN: Limpiar checkboxes ---
                if hasattr(self, 'permisos_checkboxes'):
                    for checkbox in self.permisos_checkboxes.values():
                        checkbox.setChecked(False)
    ...
    def toggle_all_permissions(self, is_admin_checked):
        ...
        Habilita/deshabilita y marca/desmarca todos los demás permisos.
        """
        if not hasattr(self, 'permisos_checkboxes'):
            return # Salir si los checkboxes no están listos

        for clave, checkbox in self.permisos_checkboxes.items():
            if clave == 'is_admin':
                continue # No te deshabilites a ti mismo

            # Si 'is_admin' está marcado, deshabilita y marca los otros
    ...
    def aplicar_estilo_actual(self):
        """Aplica el stylesheet combinado a la aplicación y fuerza la actualización de widgets."""
        ...
        # --- CORRECCIÓN: Forzar actualización de los botones de navegación ---
        ...
        # Forzar actualización de la barra superior si existe
...
# --- MÉTODOS DE RESPALDO Y RESTAURACIÓN ---
        ...
        try:
            ...
            if file_path:
                if os.path.exists(DB_NAME):
                    ...
                    mostrar_info(f"✅ Respaldo creado exitosamente en:\n{file_path}\n\nGuarda este archivo en un lugar seguro (como tu OneDrive).")
                else:
                     mostrar_error("CRÍTICO: No se encuentra el archivo de base de datos activo para respaldar.")
    ...
    def restaurar_base_datos(self):
        ...
        reply = QMessageBox.warning(self, "⚠️ Confirmar Restauración",
                                    "ADVERTENCIA: Al restaurar una base de datos, SE BORRARÁ TODA LA INFORMACIÓN ACTUAL y será reemplazada por la del respaldo.\n\n"
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if file_path:
                 try:
                    # Verificar que sea un archivo SQLite válido (básico)
                    ...
                    QMessageBox.information(self, "Restauración Exitosa", 
                                            ...
                                            "La aplicación se cerrará ahora para asegurar la integridad de los datos.\n"
                                            "Por favor, inicie el sistema nuevamente.")
                    # Cerrar la aplicación para forzar recarga limpia de datos
                    sys.exit(0) 

                 except Exception as e:
                     mostrar_error(f"Error al restaurar la base de datos:\n{e}\n\nAsegúrese de que el archivo seleccionado sea un respaldo válido y que el sistema tenga permisos de escritura.")
    
    def on_check_for_updates_button_click(self):
        """
        Se ejecuta al presionar el botón "Buscar Actualizaciones" en Configuración.
        Realiza una comprobación síncrona.
        """
        try:
            # Deshabilitar el botón para evitar clics dobles
            self.btn_check_updates.setEnabled(False)
            self.btn_check_updates.setText("Buscando...")
            QApplication.processEvents() # Actualizar UI

            # (Asegúrate de tener requests y APP_VERSION_URL definidos al inicio de tu script)
            ...
            if latest_version and download_url:
                # (Asegúrate de tener APP_CURRENT_VERSION definida)
                if latest_version != APP_CURRENT_VERSION:
                    print(f"¡Nueva versión encontrada! {latest_version}")
                    # Llamar a la función que pregunta al usuario (ya existe)
                    prompt_update(self, latest_version, download_url)
                else:
                    print("El sistema ya está actualizado.")
                    mostrar_info("El sistema ya está actualizado a la última versión.")
            else:
                mostrar_error("El archivo de versión en el servidor tiene un formato incorrecto.")

        except requests.exceptions.RequestException as e:
            mostrar_error(f"Error al buscar actualizaciones:\nNo se pudo conectar al servidor.\n\n{e}")
        except NameError as e:
            mostrar_error(f"Error de configuración: Faltan variables de actualización (APP_VERSION_URL o APP_CURRENT_VERSION) en el script.\n\n{e}")
        except Exception as e:
            mostrar_error(f"Error desconocido al procesar actualizaciones: {e}")
        finally:
            # Reactivar el botón
            self.btn_check_updates.setEnabled(True)
            self.btn_check_updates.setText("🛰️ Buscar Actualizaciones")


# ---- HOJAS DE ESTILO (QSS) ----

# --- INICIO DE MÉTODOS DE GESTIÓN DE PERSONAL ---
    ...
    def mostrar_expediente_personal(self):
        ...
        try:
            ...
            if datos:
                # (Lógica de foto copiada de mostrar_expediente_alumno)
                foto_html = ""
                ...
                <b>Teléfono:</b> {datos['telefono']} | <b>Email:</b> {datos['email']}<br>
                <b>Dirección:</b> {datos['direccion']}<br>
                <br>
                <b><u>CONTACTOS DE EMERGENCIA</u></b><br>
    ...
    def abrir_dialogo_personal(self, editar=False):
        ...
        # Aplicamos el estilo actual al diálogo
        self.style().polish(dialogo) 
        
        if dialogo.exec():
            # Si el diálogo se guardó (accept()), recargamos
            self.cargar_tabla_personal()
    ...
    def eliminar_personal(self):
        """Elimina al empleado seleccionado (con confirmación)."""
        selected_items = self.tabla_personal.selectedItems()
        if not selected_items or self.tabla_personal.currentRow() < 0:
        ...
        # (Opcional: Verificar si está ligado a un usuario)
        
        reply = QMessageBox.question(self, "Confirmar Eliminación",
                                     f"¿Está seguro que desea eliminar a:\n\n{nombre_completo} (ID: {personal_id})?\n\nEsta acción eliminará sus documentos asociados.",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                with get_db_connection() as conn:
                    cursor = conn.cursor()
                    # ON DELETE CASCADE se encargará de borrar 'documentos_personal'
    ...
    def generar_credencial_personal_pdf(self):
        """
        Genera la credencial de personal REUTILIZANDO el diseño de la credencial de alumno.
        """
        
        # --- 1. DEFINICIONES DE TAMAÑO ---
        PAGE_WIDTH, PAGE_HEIGHT = letter 
        ...
        try:
            ...
            def dibujar_cara_credencial(canvas_obj, x_offset, y_offset, es_reverso=False):
                ...
                if not es_reverso: 
                    ...
                    # --- Pie de página (Vigencia) ---
                    # (Este bloque debe estar al mismo nivel que "CÓDIGO DE BARRAS")
    ...
    # --- FIN DE MÉTODOS DE GESTIÓN DE PERSONAL ---
...
def get_accent_stylesheet(accent_name):
    ...
    if accent_name == "Azul":
        return f"""
            ...
            #navBar {{ 
                /* Se usa un fondo neutro para el menú */
            ...
            #navButton {{
                ...
                background-color: transparent;
                padding-left: 20px; /* Alineación del texto */
    ...
    return f"""
        ...
        #topBar {{ background-color: #E65100; }} /* Se usa un color más profundo */
...
style_sheet_light = """
    ...
    /* CORRECCIÓN CRÍTICA: Fuerza el color del texto a blanco para asegurar legibilidad sobre el fondo de acento saturado */
    ...
    #navBar { background-color: #F8F8F8; } /* Fondo más claro para el menú */
    
    /* Reglas base del botón de navegación (para ambos modos, los colores se aplican con el accent) */
    #navButton { 
        border: none; 
        padding: 15px 10px; 
        font-size: 11pt; 
    ...
    #shortcutIcon { 
        background-color: #F0F0F0; /* Fondo más limpio para el ícono */
        border: none; 
        border-radius: 8px; /* Menos redondeado */
        font-size: 36pt; /* Ícono un poco más pequeño, más elegante */
        padding: 10px;
    }
...
style_sheet_dark = """
    ...
    #navBar { background-color: #3A3A3A; } /* Fondo del menú ligeramente más claro para contraste */
    
    /* Reglas base del botón de navegación */
    #navButton { 
        border: none; 
        padding: 15px 10px; 
        font-size: 11pt; 
    ...
    #shortcutIcon { 
        background-color: #4A4A4A; /* Fondo más limpio para el ícono */
        border: none;
        border-radius: 8px;
        font-size: 36pt; 
        padding: 10px;
